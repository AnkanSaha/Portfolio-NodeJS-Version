...
export default async function GenerateJWT(Request: Request, Response: Response) {
	...
	const OK = new methods.Response.JSON(Response, StatusCodes.OK, 'json', 'Token JWT Generated'); // OK Response
	try {
		// Get User IP Address
		...
		// Generate JWT Token
		const JWT_Token = new methods.JWT_Manager(StringKeys.JWT_SECRET).generate(IP_Address_Details, StringKeys.JWT_EXPIRES_IN).toKen; // Create JWT Manager Instance
