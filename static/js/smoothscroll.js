 ...
 */
(function ($) {
  const h = ($.scrollTo = function (a, b, c) {
  ...
  $.fn._scrollable = function () {
    return this.map(function () {
      ...
      if (!isWin) return a
      const b = (a.contentWindow || a).document || a.ownerDocument || a
    ...
    return this._scrollable()
      .each(function () {
        ...
        $.each(g.axis.split(''), function (i, a) {
          const b = a == 'x' ? 'Left' : 'Top'
          const pos = b.toLowerCase()
          const key = 'scroll' + b
          const old = d[key]
          const max = h.max(d, a)
          if (toff) {
  ...
  h.max = function (a, b) {
    const c = b == 'x' ? 'Width' : 'Height'
    const scroll = 'scroll' + c
    if (!$(a).is('html,body')) return a[scroll] - $(a)[c.toLowerCase()]()
    const d = 'client' + c
    const html = a.ownerDocument.documentElement
    const body = a.ownerDocument.body
...
(function (b) {
  function g (a, e, d) {
    const h = e.hash.slice(1)
    const f = document.getElementById(h) || document.getElementsByName(h)[0]
    if (f) {
      a && a.preventDefault()
      const c = b(d.target)
      if (
  ...
  const i = location.href.replace(/#.*/, '')
  const c = (b.localScroll = function (a) {
  ...
  b.fn.localScroll = function (a) {
    ...
    return a.lazy
      ? this.bind(a.event, function (d) {
        const c = b([d.target, d.target.parentNode]).filter(e)[0]
        c && g(d, c, a)
      })
      : this.find('a,area')
        .filter(e)
