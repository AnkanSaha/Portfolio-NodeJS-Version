...
async function getmessage () {
  ...
  if (data.status == 'success') {
    let html = ''
    const MessageBody = document.getElementById('massagetbody')
    ...
    document.querySelectorAll('.deletebtn').forEach((selectedbtn) => {
      selectedbtn.addEventListener('click', async () => {
        const Message = selectedbtn.getAttribute('name')
        const response = await fetch('/admin/message/delete', {
          method: 'POST',
...
async function readfullmessage (id) {
  const response = await fetch('/admin/message/read', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
