...
setInterval(() => {
  const sceenSize = window.matchMedia('(max-width: 768px)')
  if (sceenSize.matches) {
    const devide = window.navigator.userAgent
    if (devide.includes('Android')) {
      document.getElementById('Version').innerText =
        'You are viewing the Mobile version on Android'
    } else if (devide.includes('iPhone')) {
      document.getElementById('Version').innerText =
        'You are viewing the Mobile version on iPhone'
    }
  } else {
    document.getElementById('Version').innerText =
